...
import type { CreateExpensesRequestBody, DeleteExpenseRequestParams, ExtractExpenseFromPhotoRequestBody, GetExpenseByIdRequestParams, GetExpensesByUserRequestQueryParams, UpdateExpenseRequestBody, UpdateExpenseRequestParams } from "./expenses.routes";
...
export class ExpensesController {
  ...
  async create(request: FastifyRequest, reply: FastifyReply) {
    ...
    const { description, amount, category, date } = request.body as CreateExpensesRequestBody;
  ...
  async update(request: FastifyRequest, reply: FastifyReply) {
    ...
    const updatedExpense = await this.expensesService.updateExpense({ id, ...data });

    // Transform and send response (convert cents back to dollars)
    return reply.status(200).send(this.transformExpenseForClient(updatedExpense));
  }
  ...
  async extractExpenseFromPhoto(request: FastifyRequest, reply: FastifyReply) {
    ...
    // Transform and send response (convert cents back to dollars)
    return reply.status(200).send(expenses.map(expense => ({
      ...expense,
      amount: expense.amount / 100,
    })) as typeof expenses);
  }
