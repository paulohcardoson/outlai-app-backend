...
import type {
	...
	DeleteExpenseDTO,
	Expense,
	ExtractExpenseFromPhotoDTO,
	GetExpensesByUserDTO,
	GetTotalsByPeriodDTO,
	MonthlyTotals,
...
export class ExpensesService {
	...
	async getTotalsByPeriod(data: GetTotalsByPeriodDTO): Promise<MonthlyTotals> {
		...
		const expenses = await this.expensesRepository.findByUserIdAndDateRange(
			userId,
			startDate,
			endDate,
		);
	...
	async extractExpensesFromPhoto(
		data: ExtractExpenseFromPhotoDTO,
	): Promise<CreateExpensesRequestBody[]> {
		const prompt = `Identifique se essa imagem é um recibo ou fatura. Caso não seja, responda com apenas o texto "error".
      Responda com um objeto despesa, com o objeto tendo os campos: description (string), amount (inteiro), category (string), date (string).
      A descrição (description) deve ser curta, com no máximo 6 palavras.
      O valor (amount) deve ser um número inteiro representando o valor em centavos (por exemplo, R$ 10,50 deve ser representado como 1050).
      A categoria (category) deve ser uma das categorias pré-definidas abaixo. Se nenhuma categoria se aplicar, utilize "Outros".
      A data (date) deve estar no formato ISO 8601 (YYYY-MM-DD).
      Caso não hava uma data explícita, utilize a data atual.
      Utilize as categorias pré-definidas: ${categories.join(", ")}.
      ...
      `;
		...
		if (!aiResponse || aiResponse === "error") {
			throw new AppError(
				"Não foi possível extrair despesas da imagem fornecida.",
				400,
			);
		}
